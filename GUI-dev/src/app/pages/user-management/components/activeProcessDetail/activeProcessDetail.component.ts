...
export class ActiveProcessDetailComponent implements OnInit {
  ...
  getPossibleUserAssignments() {
    ...
    this.assignedUsers.forEach(
      au => {
        if (!au.userId) {
          that.service.getPossibleUsersForProcessModel(au.assignedRules).subscribe(
            data => {
              const users = JSON.parse(data['_body']);
              au.assignedRules.forEach(rule => {
                that.possibleUserAssignments.push({
                  rule: rule, smId: au.smId, users: users, subjectName: au.subjectName,
                });
